...
def start_next_round(data):
    ...
    # Debug weights structure
    print(f"Round weight structure: {[w.shape for w in round_weight]}")

    layer_shape = {}
    for layer_index in range(len(round_weight)):
        if not isinstance(round_weight[layer_index], np.ndarray):
            raise ValueError(f"Layer {layer_index} is not a NumPy array. Type: {type(round_weight[layer_index])}")
        layer_shape[layer_index] = round_weight[layer_index].shape

    # Serialize weights for sending
    serialized_weights = pickle.dumps(round_weight)    
#round_weight = np.array(model.get_weights())
    #round_weight = pickle.dumps(model.get_weights())
    #round_weight = model.get_weights()

    # Check if the weights are inhomogeneous
    #try:
        # Attempt to convert the weights into a homogeneous numpy array (if possible)
     #   round_weight = np.array([w.flatten() for w in round_weight])
    #except Exception as e:
        #print("Error while processing weights:", e)
        # You might want to handle it differently depending on your logic
